-- Manually-inserted auth.users rows also need a matching auth.identities
-- row (modern Supabase Auth's multi-provider identity model) — without one,
-- GoTrue's own queries against the user fail outright ("Database error
-- querying schema"), even though the auth.users row itself looks fine.
-- Confirmed the required shape by inspecting a row created through
-- Supabase's own standard /auth/v1/signup endpoint.

DELETE FROM auth.users WHERE email = 'admin@corum-sk.gov';
DELETE FROM auth.users WHERE email = 'standardtest@gmail.com';
